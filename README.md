...
ASR 只負責把聲音轉成文字，但在實際使用中，

這些文字仍然難以閱讀、缺乏節奏，也無法直接應用在內容整理或分析。

因此我將問題重新定義為：

如何把「語音資料」轉為「可閱讀、編輯的內容」。
...
這個專案就是在解這個問題，並嘗試透過一條完整的處理流程，讓語音資料變成可用的資訊。

此專案的核心是建立一套 AI workflow 系統，

用來將原始語音轉換為「可編輯、使用」的內容。
...
使原本依賴人工處理的流程能被系統化與優化。

重點並非 ASR 本身，而在於如何把原始模型輸出轉為可直接進入工作流程的內容。
