...
因此本專案將整個流程拆解為多個可控步驟，使原本依賴人工處理的流程能被系統化與優化。

本專案重點並非 ASR 本身，而在於如何把原始模型輸出轉為可直接進入工作流程的內容。
...
- **流程拆解（Modular Pipeline）**  
  將語音處理拆為 ASR → 切分 → 字幕生成，避免單點失效  

- **模型解耦（Model-agnostic）**  
  不依賴單一 ASR 模型，保留替換與優化空間  

- **輸出導向（Usability-first）**  
  優先考慮字幕可讀性與可編輯性，而非原始模型輸出  

- **後處理強化（Post-processing）**  
  透過規則與邏輯補足模型輸出不足 
